...
double mbr_dist(vect<double> &min, vect<double> &max)
{
	...
	{
		if(query[i]<min[i])
			dist += (min[i]-query[i])*(min[i]-query[i]);
		else if(query[i]>max[i])
			dist += (query[i]-max[i])*(query[i]-max[i]);		// else dist same
	}
	return dist;	// minimum dist to bounding rectangle of a pt
}
...
public:
	...
	void knn_query(int k)				// k nearest neighbours to be found
	{
